export default {
  new: [
    ['#D8345F', '#E58A8A', '#CCAFAF', '#588DA8'],
    ['#000000', '#CF7500', '#F0A500', '#DBDBDB'],
    ['#FF5733', '#C70039', '#900C3F', '#511845'],
    ['#8566AA', '#6983AA', '#8EC6C5', '#F4F4F4'],
    ['#FFBD69', '#FF6363', '#543864', '#202040'],
    ['#97E5EF', '#FFF0F5', '#F8E1F4', '#EFA8E4'],
    ['#937D14', '#D2C6B2', '#EAE7D9', '#442727'],
    ['#B9EBCC', '#F2A51A', '#EA6227', '#342EAD'],
    ['#DD2C00', '#FF5722', '#B2EBF2', '#00BCD4'],
    ['#B7EFCD', '#4CD3C2', '#363636', '#FFBCBC'],
    ['#8DB1AB', '#CEE397', '#F2ED6F', '#F4E04D'],
    ['#F8EEEE', '#AEEFEC', '#F78259', '#EB4559'],
    ['#E7D39F', '#FB7B6B', '#D7385E', '#522D5B'],
    ['#D8B9C3', '#827397', '#4D4C7D', '#363062'],
    ['#10375C', '#844685', '#F3C623', '#FAF4FF'],
    ['#679B9B', '#AACFCF', '#FDE2E2', '#FFB6B6'],
    ['#FFEB99', '#A4C5C6', '#D4EBD0', '#856C8B'],
    ['#5A3F11', '#9C5518', '#EF962D', '#FAF4F4'],
    ['#00A8CC', '#005082', '#000839', '#FFA41B'],
    ['#698474', '#FFD3B6', '#FCF8F3', '#FFAAA5'],
    ['#FF7272', '#FFB385', '#FAE7CB', '#FFD31D'],
    ['#F1E7B6', '#FE346E', '#400082', '#00BDAA'],
    ['#CAE8D5', '#84A9AC', '#3B6978', '#204051'],
    ['#18B0B0', '#116979', '#DEE3E2', '#DE7119'],
    ['#D9BF77', '#D8EBB5', '#639A67', '#2B580C'],
    ['#F19292', '#CBE2B0', '#FCF7BB', '#F6D186'],
    ['#FBC490', '#CD8D7B', '#687466', '#084177'],
    ['#CCF0E1', '#F8FAB8', '#F8DC88', '#F76A8C'],
    ['#C2F0FC', '#FFFFFF', '#FFB0CD', '#FFD1BD'],
    ['#231903', '#7D5E2A', '#9DC6A7', '#D1F5D3'],
    ['#BE79DF', '#FBCFFC', '#FFFFFF', '#CFF1EF'],
    ['#FFAC41', '#FF1E56', '#323232', '#000000'],
    ['#235952', '#FFC8BD', '#FFEBD9', '#ECFBFC'],
    ['#D2FAFB', '#FE346E', '#512B58', '#2C003E'],
    ['#98D6EA', '#BAE5E5', '#F5FCC1', '#F3D1F4'],
    ['#FFE2FF', '#EFB1FF', '#7F78D2', '#481380'],
    ['#F67575', '#FFA34D', '#D4F8E8', '#1EB2A6'],
    ['#FFD868', '#F8615A', '#B80D57', '#721B65'],
    ['#F3ECB8', '#F5CAB3', '#A8D3DA', '#B590CA'],
    ['#790C5A', '#CC0E74', '#E6739F', '#FFB2A7']
  ],
  trendy: [
    ['#00A8CC', '#0C7B93', '#27496D', '#142850'],
    ['#63B7AF', '#347474', '#35495E', '#EE8572'],
    ['#E9B2BC', '#F3DFE3', '#F1F9F9', '#E1F2FB'],
    ['#706C61', '#E1F4F3', '#FFFFFF', '#333333'],
    ['#B9CCED', '#F6E7E6', '#FBF4F9', '#F6E5F5'],
    ['#424874', '#A6B1E1', '#DCD6F7', '#F4EEFF'],
    ['#81F5FF', '#A0FFE6', '#FFFFDD', '#FFD5E5'],
    ['#655C56', '#94D3AC', '#CCEDD2', '#EFFCEF'],
    ['#8AC6D1', '#FFE3ED', '#FFFDF9', '#BEEBE9'],
    ['#C295D8', '#E4A3D4', '#F1C6D3', '#FCE2DB'],
    ['#678A74', '#F7C5A8', '#FFEADB', '#FFBABA'],
    ['#88E1F2', '#FFD082', '#FF7C7C', '#21243D'],
    ['#DF7861', '#ECB390', '#ECDFC8', '#FCF8E8'],
    ['#6A8CAF', '#75B79E', '#A7E9AF', '#EEF9BF'],
    ['#FF2E63', '#FF9D9D', '#FFC2C2', '#010A43'],
    ['#F6EEC7', '#FFB6B9', '#F4DADA', '#BEEBE9'],
    ['#512B58', '#2A7886', '#79BAC1', '#F1F3F4'],
    ['#E9E2D0', '#EA9085', '#D45D79', '#6E5773'],
    ['#FFA372', '#ED6663', '#0F4C81', '#1B262C'],
    ['#FFAAA5', '#FFD3B6', '#DCEDC1', '#A8E6CF'],
    ['#A9FFFD', '#9399FF', '#FF80B0', '#FFC7C7'],
    ['#6C567B', '#C06C84', '#F67280', '#F8B195'],
    ['#633A82', '#72B5B7', '#40E0D0', '#F1FCFC'],
    ['#6F5A7E', '#CD6684', '#FF677D', '#FFAE8F'],
    ['#FD2EB3', '#FB8D62', '#FDD365', '#61D4B3'],
    ['#30475E', '#BA6B57', '#F1935C', '#E7B2A5'],
    ['#F3D4D4', '#FC9D9D', '#EA728C', '#4F3961'],
    ['#21BF73', '#B0EACD', '#F9FCFB', '#FD5E53'],
    ['#FFD868', '#F8615A', '#B80D57', '#721B65'],
    ['#ECCE6D', '#B0A160', '#434E52', '#5B8C85'],
    ['#96D1C7', '#F5C3BC', '#E89DA2', '#C9485B'],
    ['#FFBD69', '#FE346E', '#B21F66', '#381460'],
    ['#FFF591', '#A3F7BF', '#05DFD7', '#F35588'],
    ['#F67575', '#FFA34D', '#D4F8E8', '#1EB2A6'],
    ['#FFE2FF', '#EFB1FF', '#7F78D2', '#481380'],
    ['#FFF2E5', '#FFD5AB', '#EB9788', '#B7472A'],
    ['#9818D6', '#FF5151', '#FFA41B', '#EDEDED'],
    ['#235952', '#FFC8BD', '#FFEBD9', '#ECFBFC'],
    ['#FAAFFF', '#BBCFFF', '#F0EFEF', '#192965'],
    ['#527318', '#899857', '#FEB72B', '#FFE75E']
  ],
  popular: [
    ['#EEEEEE', '#00ADB5', '#393E46', '#222831'],
    ['#6A2C70', '#B83B5E', '#F08A5D', '#F9ED69'],
    ['#95E1D3', '#EAFFD0', '#FCE38A', '#F38181'],
    ['#EAEAEA', '#FF2E63', '#252A34', '#08D9D6'],
    ['#FC5185', '#F5F5F5', '#3FC1C9', '#364F6B'],
    ['#FFFFD2', '#FCBAD3', '#AA96DA', '#A8D8EA'],
    ['#71C9CE', '#A6E3E9', '#CBF1F5', '#E3FDFD'],
    ['#40514E', '#11999E', '#30E3CA', '#E4F9F5'],
    ['#8785A2', '#F6F6F6', '#FFE2E2', '#FFC7C7'],
    ['#112D4E', '#3F72AF', '#DBE2EF', '#F9F7F7'],
    ['#61C0BF', '#BBDED6', '#FAE3D9', '#FFB6B9'],
    ['#53354A', '#903749', '#E84545', '#2B2E4A'],
    ['#FFDE7D', '#F6416C', '#F8F3D4', '#00B8A9'],
    ['#ABEDD8', '#46CDCF', '#3D84A8', '#48466D'],
    ['#311D3F', '#522546', '#88304E', '#E23E57'],
    ['#A5DEE5', '#E0F9B5', '#FEFDCA', '#FFCFDF'],
    ['#FFAAA5', '#FFD3B6', '#DCEDC1', '#A8E6CF'],
    ['#14FFEC', '#0D7377', '#323232', '#212121'],
    ['#CCA8E9', '#C3BEF0', '#CADEFC', '#DEFCF9'],
    ['#355C7D', '#6C5B7B', '#C06C84', '#F67280'],
    ['#3E4149', '#444F5A', '#FF9999', '#FFC8C8'],
    ['#FF165D', '#FF9A00', '#F6F7D7', '#3EC1D3'],
    ['#FFD460', '#F07B3F', '#EA5455', '#2D4059'],
    ['#521262', '#6639A6', '#3490DE', '#6FE7DD'],
    ['#99DDCC', '#F6F6F6', '#FFE2E2', '#BAD7DF'],
    ['#EDB1F1', '#D59BF6', '#9896F1', '#8EF6E4'],
    ['#CABBE9', '#FFCEF3', '#FDFDFD', '#A1EAFB'],
    ['#FF5722', '#EEEEEE', '#00ADB5', '#303841'],
    ['#C4EDDE', '#7AC7C4', '#F73859', '#384259'],
    ['#1E2022', '#52616B', '#C9D6DF', '#F0F5F9'],
    ['#FF8C94', '#FFAAA6', '#FFD3B5', '#DCEDC2'],
    ['#283C63', '#928A97', '#FBE8D3', '#F85F73'],
    ['#FF7E67', '#FAFAFA', '#A2D5F2', '#07689F'],
    ['#FECEA8', '#FF847C', '#E84A5F', '#2A363B'],
    ['#769FCD', '#B9D7EA', '#D6E6F2', '#F7FBFC'],
    ['#FFAAA5', '#FFD3B6', '#FDFFAB', '#A8E6CF'],
    ['#E8FFE8', '#A6FFF2', '#74F9FF', '#00E0FF'],
    ['#A6D0E4', '#F9FFEA', '#FFECDA', '#D4A5A5'],
    ['#FF9DE2', '#8C82FC', '#B693FE', '#7EFFDB'],
    ['#F57170', '#F5F5F5', '#10DDC2', '#15B7B9']
  ],
  random: [
    ['#FBE555', '#FB9224', '#F45905', '#2A1A5E'],
    ['#FDB44B', '#448EF6', '#005792', '#00204A'],
    ['#E14594', '#7045AF', '#2B3595', '#182952'],
    ['#1A3263', '#F5564E', '#FAB95B', '#E8E2DB'],
    ['#F2F2F2', '#F0E19E', '#FC4442', '#3E333F'],
    ['#35635B', '#529471', '#A3CD9E', '#E5F1E3'],
    ['#228291', '#6796E5', '#4EC9E1', '#89FAD0'],
    ['#E9EA77', '#BBBBBB', '#297CA0', '#46185F'],
    ['#C50000', '#E70000', '#F1F1F1', '#FFFDEF'],
    ['#94F3E4', '#37AA9C', '#333F44', '#1A1A1B'],
    ['#7A4D1D', '#216353', '#75DAAD', '#EDFFEA'],
    ['#BFCFFF', '#C8E7ED', '#FFFFC2', '#FFA5A5'],
    ['#9656A1', '#C2B0C9', '#CCCCCC', '#F4EFD3'],
    ['#F2F2F2', '#FECD51', '#9F1E49', '#2F1B41'],
    ['#FDE9C9', '#EA5455', '#2D4059', '#343434'],
    ['#F2F5B1', '#E5C955', '#E08932', '#C24242'],
    ['#FFE559', '#FFBE5B', '#ED6D54', '#DD4747'],
    ['#B5525C', '#105E62', '#6BC5D2', '#D2FAFB'],
    ['#F6C667', '#B30753', '#280F34', '#BFF4ED'],
    ['#810749', '#F04F4F', '#FAE98F', '#FAA9DF'],
    ['#808B97', '#78B7BB', '#FFF1C1', '#FBFBFB'],
    ['#7B3C59', '#FFB563', '#D53939', '#3C3B5C'],
    ['#CFE3FF', '#FCCE9E', '#FD72AD', '#A561FF'],
    ['#FCB2BF', '#CF56A1', '#8B2F97', '#561F55'],
    ['#F0F0F0', '#F9E75E', '#FF5F5F', '#333366'],
    ['#866770', '#5E4949', '#C96868', '#C1E8DA'],
    ['#F8B195', '#F67280', '#C06C84', '#6C567B'],
    ['#E6F99D', '#7FDA89', '#259073', '#041122'],
    ['#EAE7E7', '#F9F8EB', '#76B39D', '#155E63'],
    ['#FFEFA1', '#E67FA2', '#6B55AE', '#6EA7C1'],
    ['#A7FF83', '#17B978', '#086972', '#071A52'],
    ['#A75377', '#D3775D', '#DFB361', '#F5EB82'],
    ['#E1BF7F', '#C65F63', '#84577C', '#333644'],
    ['#FFCDB5', '#A1C45A', '#FFE5AB', '#FFF6F6'],
    ['#23033C', '#088C6F', '#1DCD9F', '#FFE037'],
    ['#EDE9A3', '#D7C37A', '#55A44E', '#1B8057'],
    ['#A2EF44', '#31AA75', '#FCD47D', '#B23256'],
    ['#D15A7C', '#EE8972', '#F7D695', '#F6EEC7'],
    ['#4F81C7', '#64C4ED', '#FCFAFA', '#F0D78C'],
    ['#FF8A5C', '#49BEB7', '#BADFDB', '#FCF9EA']
  ]
}
